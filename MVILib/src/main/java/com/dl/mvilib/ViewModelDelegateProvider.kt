 ...
 */
@PublishedApi
internal inline fun <T, reified VM : BaseViewModel<S>, reified S : BaseUIState> viewModelDelegateProvider(
    viewModelClass: KClass<VM>,
    crossinline keyFactory: () -> String,
    existingViewModel: Boolean,
    noinline viewModelProvider: (stateFactory: MVIStateFactory<VM, S>) -> VM
): MavericksDelegateProvider<T, VM> where T : Fragment, T : MVIView {
    return object : MavericksDelegateProvider<T, VM>() {
        ...
        override operator fun provideDelegate(
            ...
            return MVI.viewModelDelegateFactory.createLazyViewModel(
                stateClass = S::class,
                fragment = thisRef,
...
interface ViewModelDelegateFactory {
     ...
     * a state factory.
     */
    fun <S : BaseUIState, T, VM : BaseViewModel<S>> createLazyViewModel(
        fragment: T,
        ...
        stateClass: KClass<S>,
        existingViewModel: Boolean,
        viewModelProvider: (stateFactory: MVIStateFactory<VM, S>) -> VM
    ): Lazy<VM> where T : Fragment, T : MVIView
}
 ...
 *
 * ViewModels are created with a [RealMVIStateFactory].
 *
...
class DefaultViewModelDelegateFactory : ViewModelDelegateFactory {
    override fun <S : BaseUIState, T, VM : BaseViewModel<S>> createLazyViewModel(
        fragment: T,
        viewModelProperty: KProperty<*>,
        viewModelClass: KClass<VM>,
        keyFactory: () -> String,
